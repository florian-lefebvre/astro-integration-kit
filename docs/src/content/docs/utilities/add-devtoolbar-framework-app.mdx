...
`addDevToolbarFrameworkApp` initialises your framework component with three props: `canvas`, `eventTarget`, and `renderWindow`.

`canvas` is the root ShadowDOM node that holds your whole application.

`eventTarget` allows you to set and listen to events from the server. See [the Client-side Events docs for more info](https://docs.astro.build/en/reference/dev-toolbar-app-reference/#client-side-events).
...
<Tabs>
    <TabItem label="React">
        ...
        export default function App({ canvas, eventTarget, renderWindow }) {
            console.log(canvas, eventTarget, renderWindow)
    ...
    <TabItem label="Vue">
        ```vue title="integration/my-app.vue"
        <script setup>
        const props = defineProps(['canvas', 'eventTarget', 'renderWindow'])
